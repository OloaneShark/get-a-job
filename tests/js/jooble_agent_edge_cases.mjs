...
const agentPath =
  `${root}/browser_extensions/jobfinitum_chrome_agent/jooble_agent.js`;
const source = await readFile(agentPath, "utf8");
const startupIndex = source.lastIndexOf("\n  run();");
const closeIndex = source.lastIndexOf("\n})();");

assert.notEqual(startupIndex, -1, "Could not locate the Jooble startup call.");
assert.notEqual(closeIndex, -1, "Could not locate the Jooble closure.");
...
const instrumentedSource = [
  ...
  `\n  self.__joobleTestHooks = {${hooksToExpose.join(",")}};`,
...
const locationState = {
  href: "https://jooble.org/away/123",
  hash: "",
  hostname: "jooble.org",
  pathname: "/away/123",
  search: "",
...
new vm.Script(instrumentedSource, {
  filename: "jooble_agent.js",
}).runInContext(sandbox);
const hooks = sandbox.__joobleTestHooks;

function resetState() {
  sandbox.document.title = "Jooble listing";
  ...
  locationState.href = "https://jooble.org/away/123";
  locationState.hash = "";
  locationState.hostname = "jooble.org";
  locationState.pathname = "/away/123";
...
function taskResponder(message) {
  if (message.type === "jobfinitum-task") {
    return {
      ok: true,
      task: {
        adapter: "jooble_resolver",
        target_url: "https://jooble.org/away/123",
      },
    };
  }
  return {ok: true};
...
await edgeCase("only external HTTP employer targets are accepted", async () => {
  assert.equal(hooks.isExternalUrl("/away/456"), false);
  assert.equal(hooks.isExternalUrl("https://www.jooble.org/away/456"), false);
...
await edgeCase("the real Continue control beats unrelated actions", async () => {
  const signIn = new MockElement({
    text: "Sign in",
    href: "https://jooble.org/auth/login",
  });
  const privacy = new MockElement({
    text: "Privacy",
    href: "https://jooble.org/privacy",
  });
  const apply = new MockElement({
    text: "Continue",
    href: "https://jooble.org/away/123",
  });
  documentState.controls = [signIn, privacy, apply];

  assert.equal(hooks.continueIntentScore(signIn), 0);
  assert.equal(hooks.continueIntentScore(privacy), 0);
  assert.equal(hooks.continueControl(), apply);
});

await edgeCase("hidden and disabled controls are ignored", async () => {
  const hidden = new MockElement({text: "Continue", hidden: true});
  const disabled = new MockElement({text: "Apply", disabled: true});
  const visible = new MockElement({text: "View job"});
...
await edgeCase("meta refresh destinations are parsed safely", async () => {
  documentState.meta = new MockElement({
    attributes: {
      content: "0; URL='https://jobs.lever.co/example/role/apply'",
    },
  });
  assert.equal(
  ...
  documentState.meta = new MockElement({
    attributes: {content: "5"},
  });
  assert.equal(hooks.metaRefreshTarget(), null);
});

await edgeCase("the Jooble index is treated as an expired posting", async () => {
  locationState.href = "https://jooble.org/searchresult";
  locationState.pathname = "/searchresult";
  assert.equal(hooks.isJobsIndex(), true);
  assert.equal(hooks.postingClosed(), true);
});

await edgeCase("a meta refresh remains in the tracked tab", async () => {
  locationState.hash = launchHash({batch: true});
  documentState.meta = new MockElement({
    attributes: {
      content: "0;url=https://jobs.greenhouse.io/example/jobs/123",
    },
  });
  ...
  assert.equal(
    ...
    "https://jobs.greenhouse.io/example/jobs/123"
  );
  assert.equal(
  ...
  assert.deepEqual(
    sentMessages.slice(0, 3).map((message) => message.type),
    [
...
await edgeCase("an access block becomes Manual Apply and advances the batch", async () => {
  locationState.hash = launchHash({batch: true});
  ...
  assert.equal(result.payload.detail.resolver, "jooble_browser_agent");
  assert.equal(
    locationState.replacements.at(-1),
...
await edgeCase("visible human verification pauses without closing the tab", async () => {
  locationState.hash = launchHash({batch: true});
...
await edgeCase("the live Cloudflare interstitial is verification", async () => {
...
await edgeCase("an expired listing reports Closed and advances the batch", async () => {
  locationState.hash = launchHash({batch: true});
  locationState.href = "https://jooble.org/searchresult";
  locationState.pathname = "/searchresult";
  messageResponder = taskResponder;
...
await edgeCase("a stripped launch fragment is restored from the background", async () => {
...
if (failures.length) {
  throw new Error(`${failures.length} Jooble edge-case test(s) failed.`);
}
